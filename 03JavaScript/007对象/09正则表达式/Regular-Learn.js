...
// 分别用于指定全局匹配、区分大小写的匹配和多行匹配。ECMAScript 标准化之前
// 不支持 m 属性。如果 pattern 是正则表达式,而不是字符串,则必须省略该参数。
...
// 只是当 pattern 是正则表达式时,它只返回 pattern
// 而不再创建一个新的 RegExp 对象。


// 抛出
// SyntaxError - 如果 pattern 不是合法的正则表达式
// 或 attributes 含有 "g"、"i" 和 "m" 之外的字符,抛出该异常。
// TypeError - 如果 pattern 是 RegExp 对象
